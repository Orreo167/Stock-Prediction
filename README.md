...
基于 **ARIMA-LSTM 混合模型** 的 A 股收盘价预测 Web 应用。输入股票代码即可自动拉取日 K 线、训练 5 个模型（ARIMA / LSTM / SVR / BP / ARIMA-LSTM），对比测试集表现，并给出未来 5–60 天（默认 30 天）的收盘价预测。
...
- 两张核心图表：
  1. **测试期预测对比图**：真实收盘价 vs 各模型；鼠标悬停时悬浮框会用边框高亮"实际值"与"最优模型"
  2. **未来预测图**：GARCH 波动率蒙特卡洛——灰色预测曲线 + 5%/95% 置信带 + 只保留最接近预测曲线的一条红色模拟路径（悬停可查看模拟路径具体数值）
...
- 高级选项：可调数据天数上限（1000–3000）、训练/测试划分比例（50%–90%）、模型启用开关、GARCH 随机种子，以及 ARIMA/LSTM/SVR/BP/ARIMA-LSTM 各模型参数；按钮旁淡红色提示"修改参数可能导致预测效果变差并触发重新训练，推荐使用默认值"，修改后自动触发重训，点击"开始预测"会自动收起面板
- 训练进度：阶段级进度条 + 动态加载动画
- 页面顶部免责声明："本系统的预测结果仅为参考。"
- 页面底部数据来源：初始展示全部可选来源，预测结束后更新为本次实际使用的来源
- 内置示例数据 `data/003015.csv`，离线或拉取失败时可演示

## 界面预览

![首页：查询、历史记录下拉、高级选项入口与免责声明](docs/assets/ui_initial.png)

![高级选项：通用参数与各模型参数](docs/assets/ui_advanced%20options.png)

![测试期预测对比图：真实值 vs 各模型，悬浮框高亮真实值与最优模型](docs/assets/ui_test%20prediction%20comparison.png)

![未来预测图：灰色预测曲线 + 红色模拟路径 + 置信带](docs/assets/ui_future%20prediction.png)
...
- **SVR**：RBF 核，C=100 / epsilon=0.1 / gamma=scale，前一日收盘价预测次日
...
- 注意：后端必须在你本机正常启动（非沙箱/代理限制环境），各在线数据源才能联网拉取
